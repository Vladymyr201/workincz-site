...
class AuthManager {
  ...
  subscribe(options = {}) {
    ...
    // СНАЧАЛА проверяем демо-авторизацию в localStorage
    const demoAuth = this.checkDemoAuth();
    if (demoAuth) {
      console.log(`[AuthManager] ${id}: СРАЗУ обнаружена демо-авторизация для ${demoAuth.email}`);
      handled = true;
      this.currentUser = demoAuth;
      onLoggedIn(demoAuth);
      return; // Выходим, не подписываемся на Firebase Auth
    }
