...
class DevLogin {
  ...
  async signInDevUser(user) {
    ...
    // Устанавливаем глобальные переменные для совместимости с DashboardAuth и RoleMiddleware
    ...
    // Устанавливаем текущую роль в enhancedAuthSystem
    if (window.enhancedAuthSystem) {
      window.enhancedAuthSystem.currentRole = role;
      console.log(`🛠️ Установлена роль в enhancedAuthSystem: ${role}`);
    }
  }
  ...
  redirectToDashboard(role) {
    ...
    if (dashboardUrl) {
      ...
      console.log(`🛠️ Dev редирект на: ${url.toString()}`);
