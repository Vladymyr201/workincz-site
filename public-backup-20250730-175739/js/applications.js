...
class ApplicationsManager {
  ...
  async init() {
    // Подключаемся к SimpleAuth
    if (window.simpleAuth) {
      // Ждем инициализации SimpleAuth
      if (!window.simpleAuth.isInitialized) {
        let attempts = 0;
        const maxAttempts = 30;
        while (!window.simpleAuth?.isInitialized && attempts < maxAttempts) {
      ...
      this.auth = window.simpleAuth;
      
      console.log('🔥 ApplicationsManager подключен к SimpleAuth');
      
      // Подписываемся на изменения аутентификации через SimpleAuth
      this.unsubscribeAuth = window.simpleAuth.onAuthStateChanged((user) => {
        this.currentUser = user;
        if (user) {
          // Для SimpleAuth используем данные пользователя напрямую
          this.currentUser.userData = {
            subscription: { type: 'basic' }, // По умолчанию basic
  ...
  async submitQuickApplication() {
    ...
    try {
      const applicationData = {
        ...
        applicantId: this.currentUser.id || this.currentUser.uid,
  ...
  async getApplicationStats() {
    try {
      ...
      console.log('📊 Статистика заявок получена:', stats);
      return stats;
    } catch (error) {
  ...
  async updateApplicationStats() {
    try {
      ...
      // Для SimpleAuth просто обновляем локальные данные
      if (this.currentUser.userData.stats) {
        this.currentUser.userData.stats.applicationsThisMonth++;
      }
    }
  ...
  async loadUserApplications() {
    try {
      if (!this.currentUser) {
        console.log('Пользователь не авторизован, возвращаем пустой массив');
        return [];
      }
      ...
      applications.forEach(doc => {
        ...
        // Используем id для SimpleAuth вместо uid
      ...
      console.log(`📋 Загружено ${userApplications.length} заявок пользователя`);
