// 🛡️ RoleMiddleware - система контроля доступа по ролям (v8.0.1)
// Проверяет права доступа пользователей к различным страницам

console.log('🛡️ RoleMiddleware: загружен для', window.location.pathname);
...
class RoleMiddleware {
  ...
  async checkAccess() {
    ...
    let profile = window.authManager?.getCurrentProfile();
    
    // FALLBACK: Если AuthManager не работает, проверяем Firebase Auth напрямую
    if (!user && firebase && firebase.auth) {
      console.log('🔍 AuthManager не вернул пользователя, проверяем Firebase Auth напрямую');
      user = firebase.auth().currentUser;
      
      if (user) {
        console.log('✅ Пользователь найден через Firebase Auth напрямую:', user.uid);
        // Пытаемся получить профиль из Firestore
        try {
          const userDoc = await firebase.firestore().collection('users').doc(user.uid).get();
          profile = userDoc.data();
          console.log('✅ Профиль получен из Firestore:', profile);
        } catch (error) {
          console.warn('⚠️ Ошибка получения профиля из Firestore:', error);
        }
      }
    }
