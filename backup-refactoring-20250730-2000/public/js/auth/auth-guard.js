/**
 * AuthGuard - Система защиты страниц от неавторизованного доступа
...
class AuthGuard {
    ...
    async init() {
        try {
            console.log('AuthGuard: Начало инициализации...');
            
            // Ждем инициализации Firebase
            await this.waitForFirebase();
            console.log('AuthGuard: Firebase готов');
            
            this.auth = firebase.auth();
            
            // Подписываемся на изменения состояния аутентификации
            this.auth.onAuthStateChanged((user) => {
                console.log('AuthGuard: Изменение состояния авторизации:', user ? user.email : 'null');
                this.currentUser = user;
                this.isInitialized = true;
                this.handleAuthStateChange(user);
            }, (error) => {
                console.error('AuthGuard: Ошибка при отслеживании состояния авторизации:', error);
            });
        ...
        return new Promise((resolve, reject) => {
            ...
            const checkFirebase = () => {
                ...
                if (typeof firebase !== 'undefined' && firebase.auth) {
                    console.log('AuthGuard: Firebase найден на попытке', attempts);
                    resolve();
                } else if (attempts >= maxAttempts) {
                    reject(new Error('Firebase не инициализирован после 5 секунд'));
                } else {
    ...
    handleAuthStateChange(user) {
        if (user) {
            ...
            console.log('AuthGuard: Пользователь авторизован', user.email);
    ...
    checkRedirectFromAuthPages() {
        ...
        const authPages = ['/auth/login.html', '/auth/register.html', '/auth/confirm.html'];
        
        if (authPages.includes(currentPath)) {
            // Если пользователь авторизован и находится на странице входа, перенаправляем на дашборд
            console.log('AuthGuard: Перенаправление с страницы авторизации на дашборд');
    ...
    checkPageProtection() {
        ...
        if (protectedPages.includes(currentPath)) {
            ...
            console.log('AuthGuard: Попытка доступа к защищенной странице без авторизации');
    ...
    isAuthenticated() {
        ...
        console.log('AuthGuard: Проверка авторизации:', result, 'isInitialized:', this.isInitialized, 'currentUser:', this.currentUser ? this.currentUser.email : 'null');
        return result;
    }
