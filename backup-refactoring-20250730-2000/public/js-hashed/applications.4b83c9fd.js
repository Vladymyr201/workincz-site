...
class ApplicationsManager {
  ...
  async getApplicationStats() {
    ...
    // Возвращаем демо-статистику при ошибке (соответствует реальным данным)
    return { total: 3, active: 1, pending: 1, accepted: 1, rejected: 0 };
  }
  }
  ...
  async loadUserApplications() {
    ...
    if (!this.db) {
      ...
      // Загружаем демо-данные (соответствуют реальным данным из Firestore)
    ...
    try {
      ...
      // Используем where для фильтрации на сервере
      const applications = await this.db.collection('applications')
        .where('applicantId', '==', this.currentUser.uid)
        .limit(20)
        .get();
      ...
      // Если не нашли по applicantId, пробуем по userId
      if (userApplications.length === 0) {
        console.log('📊 loadUserApplications: Пробуем поиск по userId...');
        const applicationsByUserId = await this.db.collection('applications')
          .where('userId', '==', this.currentUser.uid)
          .limit(20)
          .get();
        
        applicationsByUserId.forEach(doc => {
          const app = { id: doc.id, ...doc.data() };
          userApplications.push(app);
        });
      }
